...
int WINAPI WinMain(HINSTANCE, HINSTANCE, LPSTR, int) {
  ...
  if (IsLocalDevServerRunning(5173)) {
    target = "http://localhost:5173?app=smith";
  } else if (FileExists(packaged_index)) {
    target = ToFileUrl(packaged_index) + "?app=smith";
  } else {
    MessageBoxA(nullptr,
                "Quantum Smith could not find a running dev server or packaged web build.\n\n"
                "Option 1: Run npm run dev\n"
                "Option 2: Run npm run build and rebuild desktop target.",
                "Quantum Smith Launcher", MB_OK | MB_ICONWARNING);
    return 1;
  }
  ...
  if (!LaunchBrowserAppMode(target)) {
    MessageBoxA(nullptr, "Unable to launch browser app mode. Please install Microsoft Edge or Chrome.",
                "Quantum Smith Launcher", MB_OK | MB_ICONERROR);
    return 1;
  }
